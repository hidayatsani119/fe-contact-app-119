...
async function handleSubmit() {
  const response = await addressUpdate(token.value, contactId, addressId, address);
  const responseBody = await response.json();
  console.log(responseBody);

  if (response.status === 200) {
    await alertSucces("Address updated successfully");
    router.replace({
...
<template>
  <div class="flex justify-center items-center mb-6">
    <h1 class="text-2xl font-bold flex items-center">
      <i class="fas fa-map-marker-alt text-primary mr-3"></i> Edit Address
    </h1>
  </div>
